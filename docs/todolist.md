...
- [ ] **Light-mode refactor.** Site is currently dark-only by deliberate choice (see
      ...
      backdrop.
- [x] **Contact form — built, deployed, and fully working as of 2026-09-07 night.**
      ...
      - [x] Verified end-to-end on the live domain via `wrangler tail` while submitting a real
            ...
            form."
      - [x] **Doug enabled Email Routing and verified the destination address the same night.**
            Re-tested the same way — a real submission watched live via `wrangler tail` — and it
            now completes with a clean `Ok`, no `send_email error` line. No code changes were
            needed once those two dashboard steps were done, confirming the implementation itself
