...
- **`--glow` flipped from powder blue (`#7fb0ff`) to a seafoam sibling of `--note-cred`'s teal (`#52d1a6`, vs. `--note-cred`'s `#4fb894`)** — Doug preferred the seafoam from the original artifact; picked a close-but-distinct shade rather than the exact same hex so the hero/credibility rows in the mobile staff-menu stay visually distinguishable. Site-wide token, so this also recolors the nav's "hero" note and the Intro segment of `LeadSheetBarShape`. `Logo.astro`'s two-tone mark (hardcoded `#7fb0ff` + `#4fb894`, independent of the token) was already pairing both hues and didn't need to change.
